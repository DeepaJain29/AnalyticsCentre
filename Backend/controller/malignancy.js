...
// GENDER - LINE CHART
// http://localhost:4200/Gender/Count/Mal
exports.GenderCountMal = async (req, res) => {
    try {
        const SQLQuery = "SELECT gender, COUNT(*) AS count, CONCAT(MONTH(entry_date), '-', YEAR(entry_date)) AS month_year FROM (SELECT gender, entry_date FROM covid_pos.malignance UNION ALL SELECT gender, entry_date FROM covid_neg.malignance) AS combined_data WHERE gender IN ('M', 'F', 'T') GROUP BY gender, month_year;";
